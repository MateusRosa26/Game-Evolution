#!/usr/bin/env node
// Recolore um tile PNG para CINZA FRIO mantendo a estrutura de valor (luminância)
// — vira âncora "orgânica + cinza" para gerar chão de subsolo coerente e barato.
// Uso: node tools/recolor-grey.mjs <in.png> <out.png>
...
const im = decodePng(readFileSync(inp));
for (let i = 0; i < im.w * im.h; i++) {
  const r = im.d[i * 4], g = im.d[i * 4 + 1], b = im.d[i * 4 + 2];
  const lum = 0.30 * r + 0.59 * g + 0.11 * b;           // valor
  im.d[i * 4] = Math.min(255, lum * 0.86);               // R baixo
  im.d[i * 4 + 1] = Math.min(255, lum * 0.93);           // G médio
  im.d[i * 4 + 2] = Math.min(255, lum * 1.04 + 6);       // B alto → cinza FRIO
}
writeFileSync(outp, png(im.w, im.h, im.d));
console.log(`ok → ${outp} (cinza frio de ${inp})`);
