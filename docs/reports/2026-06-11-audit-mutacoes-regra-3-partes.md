...
**Tarefas restantes:** ✏️ Loremaster renomeia (Aljava→*Aljava Paciente*?, Fervor-Luz→*Resplendor*?, + EN/PT do garfo); Chat A embute consertos nas defs (via [Q1]).
**✅ MESCLA NO `design/skills/CATALOGO.md` FEITA (jun/2026):** consertos da auditoria aplicados + mutações do Lote 2 adicionadas + colisões de perfil resolvidas (Lança de Gelo: Estilhaço Profundo CORTADO por colidir c/ Permafrost; Luz Sagrada: Chama Purificadora absorvida pelo Exorcismo) + cortes (Luz Duradoura, Geada Perfurante, Passo Sombrio) + renomes flagados (Resplendor, Aljava Paciente). Golpe Forte ficou com 5 (flagado p/ enxugar). **Lote 3 (Investida/Retalho/Raio/Fagulhas/Bênção) NÃO mesclado** — essas skills ainda não têm ficha no CATALOGO (entram no M3); mutações ficam no report do Lote 3. Formato do parser da wiki (`wiki/db.js` `parseMutation`) preservado.
