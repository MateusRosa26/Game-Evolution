...
| Classes | **Knight / Mage / Rogue / Priest**, base fixa + especialização emergente. Sem subclasses escolhíveis. **Classe é adquirida no mundo, não na criação (jun/2026)**: nasce **sem classe**, spawna no berço inicial (MVP: berço único = Alvorada — modelo de spawn revisado jun/2026, ver `EXPLORACAO.md`; spawn aleatório entre múltiplos berços é feature pós-MVP); rito no NPC da classe = **quest boba + gold simbólico**, entrega a arma do kit. Condutas de Caminho contam **da aquisição da classe**. **Diferenciação mecânica (jun/2026): corpo automático forte (HP/mana/regen/cap por nível) + atributos iniciais próprios + desconto de afinidade no stat principal — magia não diferencia (de-classada). Ao classar, o corpo automático recalcula retroativo ao padrão da classe** |
...
| Ritmo / Morte | Progressão **difícil**, curva **lei de potência** (`XP(n)=175·(n−1)^2.5`, calibrada bateria farm-loop jun/2026 — **NÃO exponencial**: a geométrica explode no cap-25; 1→25 em ~35 h dedicadas, últimos 5 níveis ~39% do total). Morte perde **10% da XP total** (`DEATH_XP_PENALTY`, aplicado) — **pode deslevelar**; punição cresce com o personagem. Itens **nunca perdidos em PvE**. Exceção única: contexto PvP escolhido (flag/zona) tem perda de loot — esqueleto em `DESIGN.md` ✏️ |
...
| Escopo PvP | **O eixo decide o escopo de balance (decidido — criador, jun/2026):** **GEAR** (Marca de arma/escudo) = **PvE-only** — vive na economia de loot/grind, normalizada/desligada no duelo (o que decide PvP é gear+skill+atributo). **PERSONAGEM** (Título/Caminho/Mutação/hidden-class = o BUILD) = **balanceado p/ PvP E PvE** — build é do que o PvP é feito. Implicação: Marcas de gear passam só pelo crivo PvE; traços de personagem saem com dupla medição (feel PvE + Sirlin no duelo + simetria entre arquétipos) |
...
| ~2.000 kills com golpe final abaixo de 10% do próprio HP | **Última Resposta** | I–II | +12% dano quando HP < 25% (só auto-attack) |
| 8.000 kills noturnos | **Lâmina da Vigília** | I | brilho fraco + dano extra à noite |
| Escudo bloqueou ~30.000 golpes | **Inabalável** | I–III | chance (~12%) de bloqueio total |

✏️ _tabela alimentada pelo criador conforme o bestiário/itens nascem_

> **Números calibrados (Lote 1, bateria `docs/reports/2026-06-11-bateria-marcas-lote1-tipo1.md`):** Quebra-Ossos 15k / **+12%** vs undead · Última Resposta **~2k** / **+12%** em HP<25% · Inabalável **~30k** / blockFull **~12%**. As três são **Marca de arma/escudo = PvE-only** (eixo gear) e os multiplicadores de dano (Quebra-Ossos/Última Resposta) valem **só no auto-attack** (`viaWeapon`, fila [C1]) — nunca em magia. Os thresholds caem todos na MESMA banda de tempo (~35–50 h dedicadas): a CONTAGEM se ajusta à frequência do evento (50k de bloqueio destoava; 10k de kill-clutch era inalcançável). Bônus pequenos (tempero — regra de ouro). Os "níveis I–III" da tabela são estrutura ✏️ (a bateria calibrou o valor de topo único)._
...
- O personagem nasce **sem classe**: roupas simples + arma genérica + mochila, **sem skills** (kit detalhado ✏️ em `DESIGN-ITENS.md`), e **spawna no berço inicial** (MVP: berço único = Alvorada — modelo de spawn revisado jun/2026; spawn aleatório entre múltiplos berços é feature pós-MVP. Regra de o que é um berço em `EXPLORACAO.md`: constelação T1 ao redor + os **4 NPCs de classe** + casa inicial).
...
- **Caminhos típicos:**
  ...
  4. *Intocado* — **DECIDIDO (Lote 1, jun/2026 — mago-puro):** efeito = **golpe final com MAGIA devolve ~1-2 de mana** ("a magia se alimenta" — reembolso PARCIAL, nunca net-positive, pra não eternizar o kite); condição = **vencer N combates 100% mágicos** (físico=0 na sessão, via `combat_end`). Substitui o efeito antigo ("mana regen em combate") que era **no-op** (mana já regenera em combate). Implementado na engine (`onKill:restoreMana`); magnitude + farm-loop de kite ✏️ Balancista. Ver catálogo Lote 1 §6 ⑥.
