...
- **Charneca** (sul) — **1º destino-fora (NÃO-berço; revisado jun/2026)**, vilarejo do **arco da Contaminação**. Alcançada a pé desde Alvorada pela ponte sul; livre pra ser totalmente sombria (ninguém nasce aqui). District: planície brava, névoa, musgo cinza; weenie: a colina do cemitério com a capela em ruína. Constelação T1→T2: pântano raso (Aquáticos), cemitério (Mortos-Vivos), **Catacumbas**.
- **Brumal** (nordeste) — **2º destino-fora (não-berço)**, vilarejo madeireiro, a **fronteira selvagem**. District: floresta fechada, bruma azulada; weenie: o **Pico do Dragão** ao norte (muro T4–T5 do MVP). Constelação T2→T3: Floresta Sombria, Dungeon da Floresta, Cavernas de Gelo, colinas de ogros — bioma misto na borda da serra (elementais de gelo + ursos + lobos).
...
- **Spawn de personagens novos (decidido jun/2026 · MODELO DE SPAWN REVISADO jun/2026):** o char nasce **sem classe** (modelo Rookgaard — `DESIGN-EVOLUCAO.md`). **Regra de o que É um berço: cidade de spawn = constelação T1 ao redor + os 4 NPCs de classe + casa inicial.** **No MVP há um ÚNICO berço: Alvorada** (revisado jun/2026 — antes a regra dizia Alvorada + Charneca, com spawn aleatório entre as duas). **Por quê:** dois pontos de nascimento aleatórios deveriam ser *pares equivalentes* em tom e conteúdo — mas Charneca carrega o **arco da Contaminação** e Brumal é a **fronteira selvagem T2–T3**; nascer em qualquer das duas dá uma vantagem/assimetria pequena mas real (já estar "em casa" perto do conteúdo-bandeira) **e** força uma cidade temática a também ser berço manso, aguando a identidade dela. Então **Charneca e Brumal viram destinos que o jogador conquista a pé** (não berços) — livres pra serem 100% sombria / 100% selvagem, alcançadas com nível e por escolha. **Spawn aleatório entre múltiplos berços volta como feature de update pós-MVP** (uma vila-par NOVA de Alvorada — mesmo tom manso, bioma diferente — não rebaixando Charneca/Brumal). O sistema (spawn aleatório, casa inicial por berço, 4 ritos por berço) continua valendo para N berços; o MVP só instancia 1. Os 4 **ritos de classe** (quest boba + gold — ver `DESIGN-EVOLUCAO.md`) ficam **só em Alvorada** e dobram como tutoriais de mecânica.
...
- Uma casa inicial **por berço** (no MVP, só Alvorada — modelo de spawn revisado jun/2026, ver §Estrutura); conteúdo per-character (princípio MMO padrão).
