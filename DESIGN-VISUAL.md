...
- **Reflexo de EQUIPAMENTO no sprite — escopo (decidido jun/2026):** o paper-doll acima é o **cosmético** (set de classe + dye). Do item EQUIPADO de fato: **armadura NÃO altera o corpo** (custo alto × payoff baixo — o **ícone no EquipPanel já comunica** o que se veste; normal no gênero top-down). **A ARMA reflete por ESTILO** — 1 silhueta por arquétipo (espada/machado/maça/adaga/arco/cajado…), overlay na mão **estilo Apogea, NÃO por item** (legibilidade de combate com 10% do custo). Fundação em `src/client/assets/outfit/parts.ts` (espada+escudo do knight prontos; `✏️ demais armas` = as silhuetas por estilo). **Polish pós-MVP** — no MVP o ícone do EquipPanel basta. Obter o item (compra/casa-inicial) é o que destrava ver a armadura, não o visual no corpo.
...
- [ ] **Remaster de densidade 32→64px/tile (decidido como direção FUTURA, pós-MVP — jun/2026):** o "low res" de objetos pequenos é **contagem de pixels por asset**, não a engine. Hoje `TILE_SIZE=32`+`CAMERA_ZOOM=2` (tudo esticado 2×). Subir pra `TILE_SIZE=64`+`CAMERA_ZOOM=1` é **scale-safe** (mesmo tamanho na tela / área visível / footprints — 32×2=64×1) e **não muda o feel**, só a fidelidade (render 1:1, nítido). **Código trivial; sim intocada.** Custo = **arte**: regen/reescrita do que está **abaixo de 64px nativo** (tiles, itens, props, char-figura, mobs pequenos/padded). **Mobs de figura cheia (64px) provavelmente NÃO precisam regen** (a câmera para de esticá-los) — confirmar VENDO. **Não fazer no MVP** (re-desenha tudo, trava conteúdo); MVP shippa em 32px/tile. Detalhe na memória `densidade-pixel-remaster`.
